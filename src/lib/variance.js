...
export function costItemContributors(kind, item, region = '전체', clff = '전체', biz = '전체', cmp, topN = 3) {
  const rows = [];
  for (const e of opsList(kind, region, clff)) {
    const v = view(e, clff, biz);
  ...
  const positiveTotal = rows.reduce((sum, row) => sum + row.delta, 0);
  return rows
    .sort((a, b) => b.delta - a.delta)
    .slice(0, topN)
    .map((row) => ({ ...row, share: positiveTotal ? (row.delta / positiveTotal) * 100 : null }));
}
